    ...
    function(sitecore, backbone) {
        ...
        var Condition = backbone.LayoutManager.extend({
            ...
            renumberRules: function () {
                var ruleNames = document.querySelectorAll('.sc-rule-name');
                for (var i = 0; i < ruleNames.length; i++) {
                    ruleNames[i].innerHTML = ruleNames[i].title + " " + (i+1);
                }
            }
        });
        ...
        sitecore.Factories.createBaseComponent({
            ...
            initialize: function() {
                ...
                self.segmentBuilderUrl = self.$el.data("sc-segmentbuilderurl")
                    ...
                    : null;
                var iFrame = self.$el.find(self.segmentBuilderFrame);
                if (iFrame) {
                    iFrame[0].src = self.segmentBuilderUrl;
                    iFrame.load(function() {
                        self.segmentBuilderFrameContainer = self.$el.find(self.segmentBuilderFrame)[0].contentWindow;
            ...
            setRulesXML: function(value) {
                ...
                if (self.segmentBuilderFrameContainer) {
                    self.loadRules(rulesXmlObj);
                }
                else {
                    var isLoaded = false;
                    self.$el.find(self.segmentBuilderFrame).load(function () {
                        if (!isLoaded) {
                            self.loadRules(rulesXmlObj);
                            isLoaded = true;
                        }
            ...
            loadRules: function (rulesXmlObj) {
                var self = this;
                if (self.segmentBuilderFrameContainer) {
                    var rulesXml = rulesXmlObj.value;
                    var theEvent = new Object();

define(["sitecore"], function (sc) {
    ...
    return {
        ...
        loadListDataWithFilters: function (provider, increasePagesSizeStep, searchFilters) {
            var newPageNumber = increasePagesSizeStep === 0 ? 1 : (provider.get(pageNumberProperty) + increasePagesSizeStep);
            provider.set(pageNumberProperty, newPageNumber);
            this.requestPage(provider, searchFilters);
        },

        getData: function (provider, onSuccessHandler, searchFilters) {
            var helper = this;
            var requestUrl;
            requestUrl = provider.get("dataUrl");
            ...
            if (searchFilters) {
                requestOptions.url = provider.get("dataUrl") + "?";
                provider.viewModel.postData(requestOptions, searchFilters);
            }
            else
                provider.viewModel.getData(requestOptions);
        },

        anonymize: function (provider, onSuccessHandler) {
            var helper = this;
            var requestOptions =
            {
        ...
        requestPage: function (provider, searchFilters) {

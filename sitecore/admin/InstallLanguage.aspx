...
<script runat="server">  
        ...
        try
        {
            ...
            else
            {
                ...
                if (setAsDefault)
                {
                    ...
                    {
                        CreateDefaultLanguageConfig(configFilePath);
                    }
    ...
    private void CreateDefaultLanguageConfig(string path)
    {
      var patch = new XmlDocument();

      var rootElement = patch.CreateElement("configuration");

      var sitecoreElement = patch.CreateElement("sitecore");
      var sitesElement = CreateSitesNodes(patch);
      var settingsElement = CreateSettingsNodes(patch);

      sitecoreElement.AppendChild(sitesElement);
      sitecoreElement.AppendChild(settingsElement);

      rootElement.AppendChild(sitecoreElement);
      patch.AppendChild(rootElement);
      patch.Save(path);
    }
    
    private static XmlElement CreateSitesNodes(XmlDocument patch)
    {
      var sitesElement = patch.CreateElement("sites");
      var emptyString = string.Empty;

      var websiteElement = CreateNodeWithSingleAttribute(patch, emptyString, emptyString, "site", "name", "website");
      var loginElement = CreateNodeWithSingleAttribute(patch, emptyString, emptyString, "site", "name", "login");
      var shellElement = CreateNodeWithSingleAttribute(patch, emptyString, emptyString, "site", "name", "shell");

      var languagePatch = CreateNodeWithSingleAttribute(patch, "patch", "http://www.sitecore.net/xmlconfig/",
        "attribute", "name", "language");
      languagePatch.InnerText = "en";
      var contentLangugePatch = CreateNodeWithSingleAttribute(patch, "patch", "http://www.sitecore.net/xmlconfig/",
        "attribute", "name", "contentLanguage");
      contentLangugePatch.InnerText = "en";

      websiteElement.AppendChild(languagePatch);
      loginElement.AppendChild(languagePatch.Clone());
      shellElement.AppendChild(contentLangugePatch);

      sitesElement.AppendChild(websiteElement);
      sitesElement.AppendChild(loginElement);
      sitesElement.AppendChild(shellElement);

      return sitesElement;
    }

    private static XmlElement CreateSettingsNodes(XmlDocument patch)
    {
      var settingsElement = patch.CreateElement("settings");
      var emptyString = string.Empty;
      var clientLanguageSettingElement =
        CreateNodeWithSingleAttribute(patch, emptyString, emptyString, "setting", "name", "ClientLanguage");
      var clientLanguageSettingPatch = CreateNodeWithSingleAttribute(patch, "patch",
        "http://www.sitecore.net/xmlconfig/", "attribute", "name", "value");
      clientLanguageSettingPatch.InnerText = "en";
      clientLanguageSettingElement.AppendChild(clientLanguageSettingPatch);
      settingsElement.AppendChild(clientLanguageSettingElement);
      return settingsElement;
    }

    private static XmlElement CreateNodeWithSingleAttribute(XmlDocument patch, string prefix, string namespaceUri,
      string name, string attribute, string attributeValue)
    {
      var siteElement = patch.CreateElement(prefix, name, namespaceUri);
      siteElement.SetAttribute(attribute, attributeValue);
      return siteElement;
    }

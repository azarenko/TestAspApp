...
Function Export-WebDeployParameters {
    ...
    param(  
		...
		[string]$MsDeployPath = ([IO.Path]::Combine($env:ProgramFiles, 'iis', 'Microsoft Web Deploy V3', 'msdeploy.exe'))
    )

    # Invoke msdeploy to get result
    $resolvedPath = (Resolve-Path $package).ProviderPath
    [xml]$rawParameters = & $MsDeployPath -verb:getparameters -source:package=`"$resolvedPath`"

    if(-not $rawParameters.output.parameters) {
        Write-Warning -Message "The package $Package does not accept parameters. No parameters file could be found."
        return
    }
    # Prepare output path
    ...
    $outFile = Join-Path ((Resolve-Path $OutPath).ProviderPath) "$packageName-parameters-$fileDate.$format"

    $filter = if($Required) { 
        { 
            $result = $false
            try { $result = $_.DefaultValue -eq '' }
            catch { $result = $true }  
            return $result
        }
    } else { 
    ...
    $filteredParams = { $rawParameters.output.parameters.parameter.Where($filter) }


    if($Format -eq 'json') {
        $jsonData = @{ SetParam = [System.Collections.ArrayList]::new() }
        foreach($param in (& $filteredParams)) {
            $defValue = ''
            if($param | Get-Member defaultValue -ErrorAction SilentlyContinue) {
                $defValue = $param.DefaultValue
            }
            [void]$jsonData.SetParam.Add(@{ Name = $param.Name; Value = $defValue})
        }
        $json = ConvertTo-Json -Depth 5 -InputObject $jsonData
        ...
        foreach($entry in (& $filteredParams)){
            $element = $doc.CreateElement('parameter')
            [void]$element.SetAttribute('name', $entry.name)
            $defValue = ''
            if($entry | Get-Member defaultValue -ErrorAction SilentlyContinue) {
                $defValue = $entry.defaultValue
            }
            [void]$element.SetAttribute('value', $defValue)            
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU8R8Hw/FkblyIEuhHAT5g4HnV
# oKygghL8MIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# MAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFJFVUJqF0QYmDF7LUkNefOBU
# MjfGMA0GCSqGSIb3DQEBAQUABIIBAJy498usIN7BKwdisbQTfFgPGvrnfELTBkBe
# eI/kPm8NdchILfQ8VZ9DdLPCdwQfDm6OE10KQbN/4TM6qh0WrxpXJJWXEUTigYYJ
# /Y7RUCGoMJ8H3MTkTpHdm9gSe9cAgSh6MXIDm/ALFCy7sxEKambw5geaRsCyAIiI
# kJ0mhoZ9nsdJUwBKMEssc0a0fQahyAkzlEiCGFb9YHObEfRdM+mQBCC4FsgTHHwd
# dEiI3dgqm3xw3K5eF4Z0cw/6O2WlIVCrJMt2BlIl2fKkeNIBdW17xHfP5nylTUGL
# 0TvsIMiJjH5GYEpBJtohKsC3Qim0UHdVuEz4lknR1PeSwvTPd1ahggILMIICBwYJ
...
# MTgwMjEyMDgzNDAzWjAjBgkqhkiG9w0BCQQxFgQUVUGpBWNQatht77ZCHCFbQ1iR
# iBQwDQYJKoZIhvcNAQEBBQAEggEANQLR8SqlZ31ILtICkfHJd64mEA1PxMajNX+A
# s8/28mLKpD6CdFrtXoTHTCa1xhdOcjD4ujShmughm+z1+Bp3fAD3yIdHIGRzy9d0
# 9Xc1qnRk37f5mtufY/0uPkNFBp3PpEx7wkT76IiC9cUDUrrAOo8OkdxjGSMVa57B
# xPBeYnCThd5p2SaAhHmtMdhLI4K9sMMJxybr2AvUWDlUsQCEgpghM/kX4YCLaYES
# aFEFKzdZYRRD/Z/wCtmZztXXSVG9P2X7WJPCs7QwKCdRx22O9tYvnW5kZkZLSiqn
# rFiaQSfUiFakh1XXASxVfcE1OW2x44B0lybXFTCXe5O9JucdoA==

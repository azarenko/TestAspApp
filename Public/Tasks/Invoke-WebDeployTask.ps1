...
Function Invoke-WebDeployTask {
	[CmdletBinding(SupportsShouldProcess=$true)]
	param(
		[Parameter(Mandatory=$true)]
		[ValidateSet('delete','dump','getDependencies','getSystemInfo','sync')]
		[string]$Verb,
		[ValidateScript({Test-Path $_ -Type Leaf})]
		[string]$Path = ([IO.Path]::Combine($env:ProgramFiles, 'iis', 'Microsoft Web Deploy V3', 'msdeploy.exe')),
		[System.Collections.IDictionary]$Arguments = @{}
	)

	# Run up MSdeploy
	Write-Verbose "Building WebDeploy arguments"
	$verbArg = NewWDArgument -ArgId verb -Value $verb
	$msdeployArgs = @($verbArg)
	foreach($key in $Arguments.Keys) {
		$value = $Arguments.$key
		if($value -is [System.Collections.IEnumerable]) {
			foreach($entry in $value){
				$newArg = NewWDArgument -ArgId $key -Value $entry
				$msdeployArgs += $newArg
			}
		} else {
			$newArg = NewWDArgument -ArgId $key -Value $value
			$msdeployArgs += $newArg
		}
	}
	...
	{
		$msdeployArgs += NewWDArgument -ArgId whatif
	}

	Invoke-CommandTask -Path $Path -Arguments $msdeployArgs -TaskName "WebDeploy" -WhatIf:$false
}

Function CreateWDKeyValue {
	param(
		[Parameter(Mandatory=$true)]
		[string]$Key,
		[AllowNull()]
	...
	if(-not [string]::IsNullOrWhiteSpace($Value)) {
		$safeValue = GetQuotedValue $Value -TestValues @(',')
		$result += "=$safeValue"
	}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUzQiCrQLLP/qOiufTsFFwtfCx
# MaGgghL8MIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# MAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFMKQrfFv/c1AOaAiiX+8H1ae
# nuh5MA0GCSqGSIb3DQEBAQUABIIBAHCnw4hMK09WN6OKBZPS1mGxgepduRabGzcC
# 4f9+rmUe3Fk+MUeieLz0lwt5UHVGZS43RcICSVyiwex45MvBzOl1rvG9k4PxTFO1
# 84hPTDBrWkDu6TpWGGdEhErweVzw5SOzQVyntDycCETW0sj81+t8ZHsSV+bY5qAY
# 6LEdXVQ1eqmhIBCj75ISuLf67Z2O1pyySJqxhxCozE2bBNs+dWu6kxDvsSLfK4Ja
# GLMGariqzJZWjIybd8P0Zlb8E31Wyhv2rRSycY2d7iUTcta5N0vLz4cI3ALmb8c2
# ISJUvcIlqWDENhNJBFYlcRgKE8wdO3TuN0CiEdTflx+jVku3Q0ShggILMIICBwYJ
...
# MTgwMjEyMDgzNDA0WjAjBgkqhkiG9w0BCQQxFgQUNqMLS8SRY8h/qhZby3R72iIg
# eAAwDQYJKoZIhvcNAQEBBQAEggEARWu7g6wWWAyQJfMXTF7AUZ+uZJ7LL9gRcReG
# NHkMmZg+4sGEkRARAnq4Lo6xrqVQmDhN+hjr/dOwRisFj3yXQiqNr3OQ4PsSQfwV
# Kw9Tnrr8zAD80g3H7y6NpMjz6+TXTK9iUBCQSjyCup8vZZ6Bsvqn2KVnPjH90BaM
# xxsXdcE5M/MclAeF353H5aZ5oqbMJjQTnc2RmUE/g295nmJr8id4Q5cufeeXdK+n
# 51pbacSXuISUIVzo/jxIOFYnNWDCtiMTQiT4iKTVKy1tpGT9g+MQHTy32i03Zz0b
# YLEbEGD+x4XQXkRsjIV10gbYqEFMR7O/HXuWsPb83XTYOVvKqw==

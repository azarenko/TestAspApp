...
Function Invoke-CommandTask {
    [OutputType([string])]
    [CmdletBinding(SupportsShouldProcess=$true)]
    param(
        [Parameter(Mandatory=$true)]
        [ValidateScript({ (Test-Path $_ -Type Leaf) -or (Get-command $_) })]
        [string]$Path,
        [psobject[]]$Arguments = @(),
    ...
    if($PSCmdlet.ShouldProcess($Path)){
        ...
        if($VerbosePreference) {
            $Arguments | ForEach-Object {
                Write-TaskInfo -Message $_ -Tag 'Arg' -TaskName $TaskName -Verbose
            }
        }

        try {
            Write-TaskInfo -Message $path -Tag 'Path' -TaskName $TaskName
            # Continue with errors here, so they are caught
            $errorPref = $ErrorActionPreference
            $ErrorActionPreference = 'Continue'

            # mark the error index so we report errors in
            # the output correctly
            $errorIndex = $error.Count
            & $Path $Arguments | Out-Default
            $ErrorActionPreference = $errorPref
            while($errorIndex -lt $error.Count){
                Write-Error $error[$errorIndex]
                $errorIndex++
            }
            if($LASTEXITCODE -ne 0){
                throw "Command $Path returned a non-zero exit code - ($LASTEXITCODE)"
            }
        } catch {
            Write-Error $_
        }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU0Sfu71NmTqvPIa9OYSy6SuHU
# 6ySgghL8MIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# MAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFAHSj8u31n4Gx/2gvV/HfPUC
# 8PnYMA0GCSqGSIb3DQEBAQUABIIBAA5qtuPdVH4lFV5DgOQMu1mZJm2Kl/h8yFIY
# ZjZ/3lGVEQL0vdkHOlemjY9NmMi3rY2artePHpNhTXEK/ldszj77BVszHP+foM6H
# oE1VE9/o96x0CqcDLtcSpPBWPjePcWx43Jwj9cQjavw8/yoCI0OQlwptS34aXLTE
# 24+TxUC69Q+ZV0+VwLzevFaNP7msUR6/HQJRAmNdxuvw90a/FysdJOe0ZZM00ZbV
# J8UQqNIyJ+IxGbEuT0dIARbzS/boxYM2X69bv1dsFJpVZgXHxqZ4ggNlWW20BSH/
# UaX3S9gcx5zAQaRRoJ+MZ6ET5psMNWEyiBb1i8wJjosOujWnIa2hggILMIICBwYJ
...
# MTgwMjEyMDgzMzUyWjAjBgkqhkiG9w0BCQQxFgQUpPmC1a8RZjd5iH6Chp4dSI0o
# zi8wDQYJKoZIhvcNAQEBBQAEggEAVIXKxO+beE0/ASyBnDroSEPteaYEYDZfLrBa
# OmQrofVQCrtGQgw1XAT7TBUwvhyPAr9GJsWKT48xVOceKYeYet7iikXmmIjzbbOf
# wFCm4iYAZGKYSydlZrnwSB1D/liI+lBy/9Pu2pbUYzT4Q0hMDtIrdVPjSNmNCBM5
# OMJ6dJcpjRXB566p4HikOaqM99usFjjFPiEz8svexvRFkpzDZOXxeQWm7RJ3TNBt
# rEPnIo0QjtaosDmRP15VOoxzzeKs06zLeDRwkiU9C67f1VuyW03tghQEhFvC4Z+i
# RELZh1EtzpmqeIf3gcnL9WFu0+YKWmXrBsEZ/3yO5K50eQulBg==

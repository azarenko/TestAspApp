...
function Invoke-TransformXmlDocTask {
	...
	$xdts = Get-ChildItem $XdtDirectory -Recurse -Filter "*.xdt"
	if ($xdts.Length -eq 0) {
		Write-Verbose "No XDT files found in $XdtDirectory"
	}
    $xdts | ForEach-Object {
        
		Write-Verbose "$($xdts.Length) XDT file(s) found."

        $xml = Join-Path $RootDirectoryPath (($_.FullName -replace [regex]::Escape($XdtDirectory),'') -replace ".xdt",'')

        if (!(Test-Path -path $xml -PathType Leaf)) {
            Write-Error "$xml not found. Cancelling transformation operation.";
			return;
        }
		...
		Write-TaskInfo -Tag Add -Message "Creating a backup from $xml"
		if($PSCmdlet.ShouldProcess($xml, "Backup created")) {
			Copy-Item $xml "$xml.bak"
		}

		Write-TaskInfo -Tag Update -Message "Transforming $xml with $_.FullName"
        $transf = [Microsoft.Web.XmlTransform.XmlTransformation]::new($_.FullName);
        if ($transf.Apply($xmldoc) -eq $false) {
            Write-Error "Transformation failed. Cancelling transformation operation."
			return;
        }

		Write-Verbose "Saving $xml"
		if($PSCmdlet.ShouldProcess($xml, "Applying Transformation")) {
			$xmldoc.Save($xml);
        }
    }
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUNm8UTY7tluKpDnQG95FAInsl
# KaigghL8MIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# MAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFNsG2o+TUlECXBeYo440/P6k
# WcZlMA0GCSqGSIb3DQEBAQUABIIBAHyFyGdkE7GplPdYlSsl5D+MtoOOxxWNu010
# BCYWClXdip5xAuspEtLTHkkhL4X3qY9v4kCgeoj/eVTeeyTO4M7Q2wSIXDYd8ut7
# S1U1lJ3rnyylmsTDOSV45BMg9tGOQwWW7XL8i2ei9ALfccXxMdhb+yFxADSoZDyB
# SYAZwu2zT+tIToJkCzj2egb9HARnZHMVA/jMUJWdBdDpLUcUvgLVlzfIT/+aIhpW
# IPj/HctbnHZ4Q4SZK05lPqA7WRW+RWmBhC3Et8YPCBOsJAZxO83DQRkpJ0bCRAKU
# iOUDaFqkYYzgzu12VxXYSxAH8hia0DjtWU5m+kVY8ocM83p+83ihggILMIICBwYJ
...
# MTgwMjEyMDgzMzUzWjAjBgkqhkiG9w0BCQQxFgQU3QujvBxevhemQgrFsY4Lnv9u
# AG0wDQYJKoZIhvcNAQEBBQAEggEAfpC/I6ymdl0grLiC6OXJHjYICqDKVWNYk2ui
# LpcgTEO3bITMchjqfqm1d9z7RwtF/StZTl1WhQGtsL7GC+1Mk+XghsU/fYueGveH
# 2UHdoHjm8SBqUn78ZwR8sbV5C9AGzzr1s1whjnqPgoTdraXq3IQhbTaPW6V8qk5V
# L92AVzKon1V2nQreSkSLG9Un0oIqgF/ucUZRf/R1bjUvs9vhOaPyawjgU75pzCBY
# AFpPvrO7/y6igASMRn7ZggGpm/tNS5KJhwHrpIPPC8jEzumFjXTOOyQh+piYLak8
# VqSltQ2he0zfILoOJxDnV7R5b4GjkUC/5gPZYgQgswgPJQZu8Q==

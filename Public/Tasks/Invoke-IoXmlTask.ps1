...
function Invoke-IoXmlTask  {
	[CmdletBinding(SupportsShouldProcess=$true)]
	param(
		[Parameter(Mandatory=$true,helpmessage="Root Directory Path")]
		[ValidateScript({ if(-not (Test-Path $_ -PathType Container)) { throw "Path '$_' does not exist" } else { $True } })]
		[string]$RootDirectoryPath,
		[Parameter(Mandatory=$true,helpmessage="IO XML Document")]
		[ValidateScript({ if(-not (Test-Path $_ -pathType Leaf)) { throw "Path '$_' does not exist" } else { $True } })]
		[string]$IoXmlPath
	)

	Write-TaskInfo -Tag Read -Message $IoXmlPath
    ...
    $ioXml.IOActions.IOAction | ForEach-Object {
        ...
        switch ($ioAction.action) {
            "enable" {
				Write-TaskInfo -Tag Enable -Message $ioAction.path
				if($PSCmdlet.ShouldProcess($ioAction.path, "Enabling Config File")) {
					if(Test-Path (Join-Path $RootDirectoryPath $ioAction.path)) {
						...
						Write-Error -Message "Cannot find the path '$($ioAction.path)'."
					}
				}
			}
            "disable" {
				Write-TaskInfo -Tag Disable -Message $ioAction.path
				if($PSCmdlet.ShouldProcess($ioAction.path, "Disabling Config File")) {
					if(Test-Path (Join-Path $RootDirectoryPath $ioAction.path)) {
            ...
            "delete" { 
				Write-TaskInfo -Tag Delete -Message $ioAction.path
				if($PSCmdlet.ShouldProcess($ioAction.path, "Deleting Config File")) {
					if(Test-Path (Join-Path $RootDirectoryPath $ioAction.path)) {
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUPNAJ/+8Ikn/EZY46dRwIAT57
# XMWgghL8MIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# MAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFFnQ0ZK+l2XShmJRcVDrNUL/
# 6Gg4MA0GCSqGSIb3DQEBAQUABIIBAH6CBos6kfAyNhijh5Qn060yca1MPNif6B7Q
# 6x/0lh6eF3w1m4EWAI+J+HNa/JE/RZ/eYPECt4JCBwNfXYnFRRzwjTd9VF9V3slT
# /D7Myx0M93mE1CgOj8pj4NMSbsD2GgeTo2rI2OGY9rbovm1qAfs8CjhrBmpKnxOC
# 2AVh9BTXPx9l3YIa9xZ8C2dpqJft7YbNjGbxowQLqrVjlK7emmZvtJj2tVfYJALC
# YviM9WwFlYkC4KzVqzE6hIOzAJ6Zn5m00GmyeX03bwgWO3il9/S5tXOPppc1m1OV
# 3RVWZGq45xlysobi6Zn3qZDfBta9XGBrHz+3O2NGwG+Ujohlt1ihggILMIICBwYJ
...
# MTgwMjEyMDgzMzUyWjAjBgkqhkiG9w0BCQQxFgQUgQXAfP8mTldEkoSFyoew3s+k
# iN8wDQYJKoZIhvcNAQEBBQAEggEAbxQrJIXpqQWUZCK8Y4g+h2gYry1iP/gbPaVu
# YNo+TYnx0yiaFwjIAvhw+DCR5ilWeqt2RuTnDHY78MsAHUboCdA3wnF/A7WPNJIy
# cl9vAfTRSAC6F81WIWs59W5LN7jV4xAQyAgAx2RmFZXH/5NbuYkW2GdEYCrnBq/u
# g0bOkTBB79FpTSlrZQtq8NZme3UxuTZYuDHE1yt5GcE1DYjq8kDq0j1/3eb+yvDy
# 6BLw1z0eTd/lsjuWie767It6cMV5uKKSorMNQZLnlO7SKtXqS9x3WGmoxvUjmaI0
# qrhE0f1Y5i34LEFt5sKakb5fM2ChjkK6Vee6NDpr1qTV6O67Ew==

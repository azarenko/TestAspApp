...
function Invoke-SetXmlTask {
    ...
    Write-TaskInfo -Tag Update -Message "$FilePath"
    ...
    if($found = Select-XML -Xml $xml -XPath $XPath) {
        $found.ForEach({
            ...
            if($PSCmdlet.ParameterSetName -eq 'Add'){
                ...
                if(($contextNode.ChildNodes.Count -eq 1) -and ($contextNode.ChildNodes[0].Name -eq '#text')) {
                    $contextNode.InnerText = ''
                }
                [void]$contextNode.AppendChild($newNode)
                $contextNode = $newNode
                Write-TaskInfo -Tag Add -Message "$XPath [/$Element] => $Value"
            } elseif ($PSCmdlet.ParameterSetName -eq 'Update') {
                # Check if we were actually passed a value
                $passedValue = $PSBoundParameters['Value']
                if($null -ne $passedValue) {
                    $contextNode.InnerText = $Value
                    Write-TaskInfo -Tag Update -Message "$XPath => $Value"
                }
            }
            ...
            if($Attributes.Count -gt 0){
                foreach($key in $Attributes.Keys){
                    ...
                    $contextNode.SetAttribute($key, $attrValue)
                    Write-TaskInfo -Tag Set -Message "$XPath [@$key] => $attrValue"
                }
            }
        })
        $xml = [xml]$xml.OuterXml.Replace(" xmlns=`"`"", "")
        Write-Verbose "Saving $FilePath"
        if($PSCmdlet.ShouldProcess($FilePath, "Writing Xml")) {
            $xml.Save((Resolve-Path $FilePath).ProviderPath)
        }
    } else {
        Write-Error "Could not find path: $XPath"
    }
}
# SIG # Begin signature block
# MIIXwQYJKoZIhvcNAQcCoIIXsjCCF64CAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU+Rlcs6N+0DdvQhzApDMWeW4r
# YbGgghL8MIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# MAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFE6QgErOxmnZMHZ0GAhN8bgQ
# FDPEMA0GCSqGSIb3DQEBAQUABIIBAGOy+aFejtBPWSWZou/zAwcKWJ4ESRq0YShq
# MAMT5t1pKFjGE0p5m9qf9Ah1NZEfJkJLmjMwq67OPGaxt0M7FutGx7+LUkgKk4h0
# xh3djHMQHgqoLWT8U12AgmTtc4JbB8U8NQTB7EUQpRFEnNqLDECoV3lM8aWUIIYK
# qhNyxVuYKckABlvKrbyxDJ1sM1tTYfNUIQuv2TcZpy9EDWOKaar1C7FEOS8awPOa
# miFYhHtkf8A5hVPBxS6jUDETYbbAbuL4IpC9Rw+ffn7BjRV2U5JhbIo7NTTvL+rq
# qJo3n4Gd3FWpDts0jOaAABD8CaZYCXCZAPfjlFBIMJSnSouFxZChggILMIICBwYJ
...
# MTgwMjEyMDgzMzUzWjAjBgkqhkiG9w0BCQQxFgQUSeVVpGZx7JOgKKsKlK20CI5q
# FOAwDQYJKoZIhvcNAQEBBQAEggEAKJ/aS4KuOhkzbgRibWtj3kfJJTPB+D+uN3r7
# XKyzYrT3iQ7FizCBc0uqnv6q4c35Hh2Pvz+K+YmvVt6zBXX1yQYZ04a06mwe9tEC
# FBk3/iVnNYiKTd64hKdy34pbs+K96uV6DE4V1epA63qN+e3GH14IqO+Sw4AkaqwU
# F83m+mIS1tZNS6RFJaT3Mj+SCPF6fCdwcn9ZZgySMwTbGEbzaGDfSxdE5fHkSh6j
# NAaYgECn5Rk4m/6Vusq8m0DNPApBT7XoqdyunwbhZPrTb3uFdJeMaLqbfNadjjv/
# 6IPwdO7fOJpBDVtmWzeg1HZhSqnWadVbZIwUgWBK2BhPyaAQ+A==

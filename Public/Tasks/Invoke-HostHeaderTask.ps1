...
Function Invoke-HostHeaderTask {
	...
	if(-not (Test-Path $hostsFile)){
		Write-Warning -Message 'No hosts file found, hosts have not been updated'
		return
	}

	Write-Verbose "Creating backup of hosts file to $hostsFile.backup"
	Copy-Item $hostsFile "$hostsFile.backup"
	Write-TaskInfo -Message "Created - $hostsfile.backup" -Tag 'Backup'

	# Build regex match pattern
	$pattern = '^\s*' + [Regex]::Escape($IPAddress) + '\s*' + [Regex]::Escape($HostName) + '\s*$'

	# Ensure we wrap to an array here so less than two entries still supports counting
	$existingEntries = @((Get-Content -Path $hostsFile -Encoding UTF8)) -match $pattern

	if($existingEntries.Count -eq 0)
	{
		Write-Verbose "No existing host entry found for $IPAddress with hostname '$HostName'"
		Add-Content -Path $hostsFile -Value "`n$IPAddress`t$HostName" -Encoding UTF8
		Write-TaskInfo -Message "$IPAddress => $HostName" -Tag 'Added'
	}
	else {
		Write-Verbose "Existing host entry found for $IPAddress with hostname '$HostName'"
		Write-TaskInfo -Message "$IPAddress => $HostName" -Tag 'Exists'
	}
}
# SIG # Begin signature block
# MIIXwQYJKoZIhvcNAQcCoIIXsjCCF64CAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUU6JM5vj1U0c1pYNxfkWXDVhF
# W86gghL8MIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# MAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFNMHAh02I7fd/+MRdvPj6uzr
# CrYGMA0GCSqGSIb3DQEBAQUABIIBAAzmzgEgz2ncBOM84E/HLarcks31seRwyyr0
# QYJUXK6Fz9iMLZbT8MMl5nPzQj0pB3blBEaieyNAWXJGCa5f5dz17xmgcXoaZp/A
# eVeoyZTBc+nQkpVzgYCA8El7UnXlI5pPBEox0WvDl09YBFUnfbcfPld7/LB32nLc
# AYz86S7i7fToLqcugGkcnYPN6VHI9RbmJMztfNwlq1XwRd7Mk+9GRuoqMrW3nUP+
# K5UmRvB3V2TqCwjK+23hmZgHA1Q4BgygrzZyZC+OGLxNYFlFYAl4TQETk9P3+WkY
# P0rIX1dZcrgSFSoBQy85NNT/HqsiOI+JqUkhMiF0SMjCTr6ndp2hggILMIICBwYJ
...
# MTgwMjEyMDgzNDAzWjAjBgkqhkiG9w0BCQQxFgQUyiXFcTJeNdhXAIn2/shhamiU
# t7UwDQYJKoZIhvcNAQEBBQAEggEAfx+OME0jkUsBq2rKKcUDTJGUNiR9Bpf5j/BA
# MdbZXV/vaPrLyp6WBE/lCmKbg0UPp7w0pVER5ZPf/8MSO8jtahs/GtqRTXTFRrRG
# jsB/229ehSYALH4sukT91ojtoGXbyZytOgUFbl01U5Ap7k1NDtVbdUVkT9yuTNB6
# 1w7Z/SuNrljAZ11U2zhS5Jmf0nImfOcnVpXDRywnONIWwNwfBlwNLv9ik97Y12jQ
# 5iXpsQeUEF0ong1oIPcP8rOySwLDxrc2R6Csc2URbe/udE315WmHOpTfuFBK+eiF
# 78XxcpufFAxda7tB9mwMXyIO4Kw0wCyo/UpGA8AhAprOjdB6LQ==

...
function Invoke-ManageSolrConfigTask {
    ...
    try {
        ...
        $uri = "$Address/$CoreName/config?wt=json"

        if($null -eq $Config)
        {
            Write-Verbose "Config was null"
            $Config = @{}
        }

        $data = ConvertTo-Json $Config

        try {
            if($PSCmdlet.ShouldProcess($Address, "Invoke-ManageSolrConfigTask -Address $Address -CoreName $CoreName")) {
                Write-TaskInfo -Message ("Updating config on $Address for $CoreName" | Out-String) -Tag 'SolrConfig'

                Write-Verbose "Updating Solr instance '$Address' configuration for core '$CoreName'"
                Invoke-RestMethod -Uri $uri -ContentType "application/json" -Method "POST" -Body $data -UseBasicParsing
            }
            else {
                Write-TaskInfo -Message ("Updating config on $Address for $CoreName" | Out-String) -Tag 'SolrConfig'
            }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU/J4GWhsohAj9tVnTH5gD0bq0
# TR6gghL8MIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# MAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFDNb+Pzd4NSUAZx/pnH4yEZW
# qPpBMA0GCSqGSIb3DQEBAQUABIIBACkxrxIBDreQ7k+XHMsrk+exLe7eRP8g2QO7
# ykR6tjFZfHPp+0OhWLsmWqLt/L4D14fMeaF79GLFGcrOroKi53eiVrjUwx7UdUkF
# Sg2Ia9HMq9Ou4QV2q6Ea5vZKndZ+AJefOOGD0Pdz5rSi9/MWwb3JNzPKdT+IUXGW
# JQE67Mk8SoLt5vRdGdlU77eX1b2+hWObh006osX0n7pyHzqy9xuB0P6opKCM2cTH
# HkdiqRp9rT0UDyy5blXekFP0iqy7FM3STlhgeMDAe152FJr7cAYLA/M5+859T3p/
# RsQTamZAaRAtkV87kj8zkjn+TPAwlM+Rz8UfBy29mdNTg2sq9O6hggILMIICBwYJ
...
# MTgwMjEyMDgzMzU5WjAjBgkqhkiG9w0BCQQxFgQU52nwUxCT94oHMj9acdAaGC1g
# MZYwDQYJKoZIhvcNAQEBBQAEggEAWeBBFMH74R1joJWtkcrSQe0hFctNK+AvKx+G
# MG4PYela8PG47j0g+5AAbsooH9hlRc+Kye/O7KEgF0ob7iy58sSUQRUJLuZCbW+2
# iBEZfN5Rg1EavTQq92dSsWjlPf6SkQ6UT+RVCpP0ji3mXdu5DJNIGdihnRAGHqHh
# QX4PHo1BdR6E/r0h1VaT9795eqYu6GqM176SvTueGQrShVVxYDQp38XD+/M++QE4
# F1GtMigd+BtyVpnBUOxsXtobjulujQuOCb68LKXYLeiuGSQV2BK9Zfu4sf9ixDxN
# R/IPhDBKHb7kpEthB9+Mu/MU8zs9XLN3FmtGg3U7TRCfVUArnw==

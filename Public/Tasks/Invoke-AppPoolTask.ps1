...
Function Invoke-AppPoolTask {
    ...
    if($Properties -eq $null) {
        Write-Verbose '$Properties is null. Using default: @{}'
        $Properties = @{}
    }

    $poolPath = "IIS:\AppPools\$Name"
    $instance = Get-Item -Path $poolPath -ErrorAction SilentlyContinue

    if(!$instance) {
        Write-Verbose "Creating App Pool $Name"

        if($PSCmdlet.ShouldProcess($Name, "New-WebAppPool")) {
            $instance = New-WebAppPool -Name $Name
            Write-TaskInfo -Message $Name -Tag 'Create'
        } else {
            $instance = New-Object PSObject -Property @{ PSPath = $poolPath }
        }
    } else {
        Write-Verbose "App Pool $Name already exists"
        Write-TaskInfo -Message $Name -Tag 'Exists'
    }

    # Bind
    Write-Verbose "Setting properties on $($instance.PSPath)"
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU/qG0B92zPjPXih3zQ1fVFDTU
# 2AOgghL8MIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# MAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFAZM7y4lshgtj6/bDBO9dofw
# 8UAZMA0GCSqGSIb3DQEBAQUABIIBADD0Ybs4wIOOrH73/dj0eJzu2usp1rQbbToM
# C31PfZ/PRuLd38hDheSfw/H/njt/ZS10jgYUv8jQZRvpk9SQJz9Xkp7irTWGt5pO
# AodO+ovNGzPsKjU3ruaWtIAw3gtJdxyEETyDSaQ2FPFsUqdyaLw7uhA07P+ey6f3
# n4hS4lLkzwuQz6tYEauPHboQHazdTlXxDa5c8UQZnFHjxhfi1NUsGr+sKYZUCrcQ
# 10iQ5Bl+mVsvVJh5Z5Wcu/E8Tzp8hM7wCd/vpUOLjNzPGBI/kmGoRa5jI8UQFnGt
# CkVzbsLeEk4ayTtsAyZxKg7mP/M2E3w8NRBZkznlZu7KdmmsoUGhggILMIICBwYJ
...
# MTgwMjEyMDgzMzU5WjAjBgkqhkiG9w0BCQQxFgQUmmfsAl0ucKLyAjFtTLd6YXV7
# Al4wDQYJKoZIhvcNAQEBBQAEggEANwFO75rSt9HwWOGa3FBDeL5+0u5kEKLv4M4U
# w8nBaODBadgkAodyBU6ZB3/HqB96VJ6HaVPq0lVrgZhxVjlpthpQczYA0Uo9yL3R
# 2BVRVVDEIibd9F+ybhEXxy0x3OmHdYqxt0ErBtDROLsIPjVBNYjini3FEj3VdU7S
# vKn0jfc80ae7S0h/VlV9h7ew8oxDIqSIezj0jFKx9TiM0ZsCwxO0gEDOGBeqyjzc
# 3hYX0ablkDZuaCDlmmo4rpZHlpKf19Eryu7ZoT+z1oMfLcwTW1upIbAmSiSbvenl
# rScntuTG/NGHg+s3s/dVvgg05apGzS2j8bqpoZyJ22+OxMbPUw==

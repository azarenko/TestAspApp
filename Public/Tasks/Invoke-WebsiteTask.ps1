...
Function Invoke-WebSiteTask {
    ...
    $instance = Get-Item $sitePath -ErrorAction SilentlyContinue

    if(!$instance) {
        Write-Verbose -Message "Creating site $Name"
        if($PSCmdlet.ShouldProcess($Name, "New-WebSite -Name $Name -ApplicationPool $ApplicationPool -PhysicalPath $PhysicalPath")){
            Write-TaskInfo -Message $Name -Tag 'Create'
            $instance = New-WebSite -Name $Name -ApplicationPool $ApplicationPool -PhysicalPath $PhysicalPath -HostHeader $Hostname -Port $Port -IPAddress $IPAddress
        } else {
            $instance = New-Object PSObject -Property @{ PSPath = $sitePath }
        }
    } else {
        Write-Verbose -Message "Site $Name already exists"
        Write-TaskInfo -Message $Name -Tag 'Exists'
    }

    # Add the additional parameters
    # even if new, pass so they can be updated
    $Properties['ApplicationPool'] = $ApplicationPool
    $Properties['PhysicalPath'] = $PhysicalPath

    # Bind
    Write-Verbose -Message "Setting properties on $($instance.PSPath)"
    SetPropertiesFromHash -On $instance.PSPath -Properties $Properties

    # Update Binding information (if missing)
    if($PSCmdlet.ShouldProcess($Name, "New-WebSite -Name $Name -ApplicationPool $ApplicationPool -PhysicalPath $PhysicalPath")) {
        if((Get-WebBinding -Name $Name -HostHeader $Hostname -Port $Port -IPAddress $IPAddress | Measure-Object).Count -eq 0) {
            Write-Verbose -Message "Setting binding ${IPAddress}:${Port}:${Hostname}"
            New-WebBinding -Name $Name -HostHeader $Hostname -Port $Port -IPAddress $IPAddress
        }
    }
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUQINfFqSwDFio13s29ldYOgkn
# bCmgghL8MIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# MAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFCJ/oBSN4fV3YOkeQ3sfiNm2
# yt+HMA0GCSqGSIb3DQEBAQUABIIBAH9QF8sLNIqWYB53XEvnOm0oiPRdNJPVT1/d
# Bjk73fsPnjG9ELjO6RsxeMli9roLBPhHkMoOLV5PXFCf2fOEKsRSDL39QorvDFqE
# j2bfcCT47HoeD1XPyvYLKrCCaKpvY8EhPqkEnrgpWpD78Vp92DMigcfW1IUwkebA
# y0TJN95oznPl9HxEVI59Z2PUlVOg7yHMBiH71HxtWhNeEPoVJ2G73CTVi908kNWA
# ubwzWXXjCpKNGcNEqCNDDyN0H2nKpIZLXJDEZr56rFWmz/AcJNm4SiRPOEmA8N6m
# FRwTGqytMX/kFhssh8jeVq8BSlIpPBfEfH0xU0Uw1nxLVXfAga2hggILMIICBwYJ
...
# MTgwMjEyMDgzNDAwWjAjBgkqhkiG9w0BCQQxFgQUQXQ7AxvB17AtV2hs5Rnv/REJ
# 300wDQYJKoZIhvcNAQEBBQAEggEAX7AMJlgs/RSaKuEjNkTB02l8Na4MwJtphQ2m
# sb1NE6fzKke5fZhGTpraJ5emQ1EPsZ01u8tQvjFDWbZylZ4TF5mIq6l0dJ2iiP9W
# uitVjtyJoswraWX3TuNa3T9vorGmn7qIFT0Mg/0NwdbQT3iiMFsgnS3vSLH0qyBA
# B0XsL5hMhNyS9ZpffrgyvR7fo8rcryaNnzI2iLvvA952svNmaqYtbGdQtNcT8oON
# L8Qf9eeReSW8siFSjUCsnINEbC3EtyAB03rcFPZVTLyriCfqH/8/+5yA3PQIt8kk
# T4bJTxVffhDioifv3Gb0fmMOJ1OQSFcw+05S4f3wG/OZ3rqSKg==

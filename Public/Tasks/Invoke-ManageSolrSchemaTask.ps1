...
function Invoke-ManageSolrSchemaTask {
    [CmdletBinding(SupportsShouldProcess=$true, DefaultParameterSetName='Args')]
    param(
        [Parameter(Mandatory=$true)]
        [string]$Address,
        [Parameter(Mandatory=$true)]
        [string]$Core,
        [Parameter(Mandatory=$true, ParameterSetName="Args")]
        [PSObject]$Arguments,
        [Parameter(Mandatory=$true, ParameterSetName="ArgsFile")]
    ...
    try {
        ...
        $uri = "$Address/$core/schema?wt=json"
        $body = $null
        if($PSCmdlet.ParameterSetName -eq 'Args'){
            $body = ($Arguments) | ConvertTo-Json -depth 100 -Compress
        } else {
            $body = Get-Content $ArgumentsFile | Out-String
        }

        try {
            if($PSCmdlet.ShouldProcess($Address, "Invoke-ManageSolrSchemaTask -Address $Address -Core $Core")) {
                Write-TaskInfo -Message "Modifying Schema on $Address : $Core" -Tag 'Schema'

                Write-Verbose "Updating Solr Schema: Uri => '$uri'"
                $resp = Invoke-RestMethod -Uri $uri -Body $body -Method Post -ContentType "application/json" -UseBasicParsing

                if($resp.HasMember('errors')){
                    foreach($err in $resp.Errors){
                        if($err.HasMember('errorMessages')) {
                            $err.errorMessages | Write-Error -ErrorAction 'Continue'
                        }
                    }
                    throw "One or more errors returned from Solr request."
                }

                return $resp
            }
        } catch [System.Net.WebException] {
            $err = GetSolrResponseError -Exception $_.Exception
            throw $err
        } catch {
            throw $_
        }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUhtbssTP4uA70OkT7TjBMKa63
# ePSgghL8MIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# MAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFBKnd7DaBSte4/kl5BLLyn22
# gsFTMA0GCSqGSIb3DQEBAQUABIIBAGG9aCaJjnXtUCk8AXt810GLTaABQFdQpQ1F
# VIivBS7ibSrdFxd2LJ7io6FxpywyLvqJIBy3jGrQX+LGNOpf+PsR4EQpVr6AnfZT
# cc0xhxvA1+uTWGKNi3vkrF+Qx8S9Q/Oez2mkz1mU1cc417X6SeK7Zu+G4iP2Myyi
# gZwe/9CiFQtbXf+pV86/PN1e36aWGyZWA9Qj7vG6v3qkSa91OFv94D0mNg+xQGY+
# NVxVn2jIYY6/FuC0CmatQdJlCA+9hfP0ipdMBP8K2xfF745ZdHZbNzbWJttjT7fF
# lEMlnC+F0uwDo0cHeZckdN8bIUWITpciExB+UVQchHZlfaAXYauhggILMIICBwYJ
...
# MTgwMjEyMDgzNDAyWjAjBgkqhkiG9w0BCQQxFgQUqDJbOCIhygaX97jrKVaFIS/j
# CPkwDQYJKoZIhvcNAQEBBQAEggEAG7O9VSlj7KjWfTO4VysRT494Tao1erIS2hu4
# Ga/KSRN6Kdgi0BAswHWp1A3jkWyNcvwhuOCi3yxxChhExpHfLazP9ITgf0CtavyL
# +EAsNOKEXA9NToqvWHWZQJIpZDsnK+VWBo1YYQRFhSqjsU2AcNWixso4/6eCqkQU
# 5rQVfbsdhHouYlKSJfRLb3H1BhMb1BeZNJkcbewYjDHZpvwqrh9KZIjysTq+jNom
# grjygGLAjedaoAZ4xFn10TwcoFhhll87+hD1KUinAYTQL9QQhsySTufawWMGuGd5
# nndC6MZOeO84S7uu4hxxhFjEpwH28WfNdQe1TC/Ots8LypPbUQ==

...
Function Invoke-ResolveCertificatePathConfigFunction {
    param(
    ...
    try {
        ...
        Write-Verbose "Resolving path: $CertificatePath"
        ...
        if (!$certificate.HasPrivateKey) {
            throw "Certificate $CertificatePath does not contain Private Key. Ensure that it has been exported with the Private Key."
        }

        # Resolve unique name based on RSA Privatekey
        $keyName = $null
        try {
            $keyName = GetUniqueName $certificate $CertificatePath
            if(!$keyName) { throw }
        } catch {
            throw "Could not resolve private key name for certificate $CertificatePath"
        }
        Write-Verbose "Certificate key name: $keyName"

        # Try the RSA match
        $basePath = [IO.Path]::Combine($env:ProgramData, "Microsoft", "Crypto")
        $rsaKeyPath = [IO.Path]::Combine($basePath, "RSA", "MachineKeys", $keyName)
        if(!(Test-Path $rsaKeyPath)) {
            $rsaKeyPath = [IO.Path]::Combine($basePath, "Keys", $keyName)
            if(!(Test-Path $rsaKeyPath)) {
                throw "Cannot find key file for certificate $CertificatePath"
            }
        }

        Write-Verbose "Certificate path: $rsaKeyPath"
        $rsaKeyPath
    }
    catch {
        Write-Error $_
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU3RS05WxVebqSqzS7nxoBftkT
# 8x2gghL8MIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# MAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFGtYs4zWEzWi95Ds0i6BIufi
# zk2PMA0GCSqGSIb3DQEBAQUABIIBAJZzXaTejHt2/ahGz3GjgO8l+r4+y1xXgzAx
# K1sXYJDeJSEuEc7PQqNytUCwxayJrdCXKKruteLZ0hpTxY7+RF887OyqoIkYG4D9
# aFPPLN7fxcz3/D9F3CjlsYkZyJTfQy9pNjA8Mzv0AgynUnQp6Jr0Aj0osqwKT+Pi
# g8biGR7tdXm+YiIWjl8P3Km2rz9QRt+OSeHRa18WtbUoT+dDUEevn4jAnJYZOa2a
# f5Pwe2tpPayPLlkCoxxP2joUiOKnHigzBYhupJ6qR40Uw0oAS/xaVFQSRNa5rQpo
# Bw5drCbYpYuA/+hdEIFIiS7oJdPppQDcDq04kygamDGvQQnRh/WhggILMIICBwYJ
...
# MTgwMjEyMDgzNDAyWjAjBgkqhkiG9w0BCQQxFgQU5oB1s0kuTKJLmXGlsivq5hzm
# KKkwDQYJKoZIhvcNAQEBBQAEggEAQTb5OMp4zCSFP32d5PuoT7gtdZZ+r1GtNmlP
# HAtV7t/bwkGE28mo+lSlWZgyBSQVtV2NIf7Y5TIE3HpzOeAPRc0gISov7mS5PXyE
# WI+OeApE3xhPHc3VHFttChSbT2wIWKIBzmojuasnLPIB7hUM0dXBICyn4hI0xpcZ
# Yh4XCXoQwuavB/VXkLO5a1FrWKy9q7GwHvzOJEAH/7apfHcf/gHNO6Ui+60z9dls
# USlkHTkXCyacLGl92TD4JRGf8LcBDTd5B4nCWiTCkMX3sqzMLNw422X8XznNlnZw
# K5r8O9bRHV2VGFumFBfM/0urGioA84qwK/LXkHZNcN+LVpcPag==

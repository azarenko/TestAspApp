...
function Invoke-JoinPathConfigFunction {
    param(
    ...
    function ProcessArray {
        ...
        Write-Verbose "Joining: $Array"
        $result = $Current
        if($null -ne $Array) {
            foreach($entry in $Array) {
                if($null -eq $entry) { continue }

                if($entry -is [hashtable] -or $entry -is [System.Collections.Specialized.OrderedDictionary]) { continue }

                if($entry -is [System.Collections.IEnumerable] -and $entry -isnot [string]) {
                    $result = ProcessArray -Current $result -Array $entry
                    continue
                }

                # else
                $next = $entry.ToString().Trim()

                if([string]::IsNullOrWhiteSpace($next)) { continue }
                if([string]::IsNullOrEmpty($result)) {
                    $result = $next
                } else {
                    $result = Join-Path $result $next -ErrorAction stop
                }
            }
        }
    ...
    {
        Write-Verbose '$Values is null or empty'
        $result = ""
    }

    try {
        $result = ProcessArray -Array $Values
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU/d5MR288nHfI2U0/yMsLkDjy
# /i6gghL8MIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# MAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFDYZWd/Hf3qI6jlejKGR80my
# d6p4MA0GCSqGSIb3DQEBAQUABIIBAIF8CqWTLltEQuqEuvji7XT4CANoO7myAdBm
# hbeXbSEEiNucPuk8P0BLiK2QtlnR7gm1YVJkfBlCecXPvm7uYlECDs0ylBWejZjR
# pfIKjpM3nTsVDlY9+oF12RkhrQt1ze2nW9Od2fzaLIaUNMio4l7BS/t3Nh8iSUQ7
# fQX9Q/qNX2qFvHPdDg5orCn4vADJpGuGxz9KebTv6ZDj58CHk1iovsBLCcPtmB3F
# KZplJz7hBCeiK8Lef62BGqzTELY6Rt8MlkEHAu4YJm7K1w6X/L+KXZOv+4jJGPlU
# T+TMt3qO5AqZNKukP7/cgwsN5mfPUeR+PuO7g61+BiYY1khWp9GhggILMIICBwYJ
...
# MTgwMjEyMDgzMzUwWjAjBgkqhkiG9w0BCQQxFgQUa4iwTgJt1WxOPe6p1SdwmGxQ
# izQwDQYJKoZIhvcNAQEBBQAEggEAMCfY/fkW7mj9cuA+Nezqe47Cy9kiBVqXIgah
# DyXx9i2HWIytpq7aIGi8W6KyrRGEdj93GrprLRd8ytqmF9d84l5uOvqY+OMsEHzd
# 0idLI69GI2WXA/OaXutwl15gFePErelE9wLUQObQ1szXDSFmtmaTwrMsHtzGKUBo
# XE5o+N7wsfJeJ4sobX5dWPV55YX1CKVft4peIPjOkaBJ0mf+AjPfkzAkRj8zSJaY
# wDfXGEppiN0thYeu4F+AF8ByoPQmklSqn6yratOBi6dQqFtdVb5CUKQ1CANcwvBu
# Wx6pHhABae2NPTF23Pxa1QjboII1k1I2TbRUZJ2Wqx0ecOqRDw==

...
Function GetQuotedValue {
    ...
    # break early if already quoted
    if($Value -match "\'.*\'|`".*\`""){
        return $Value
    }

    if([System.Linq.Enumerable]::Any($Value, [Func[char,bool]]{
        param($x) ($TestValues -contains $x) -or [char]::IsWhiteSpace($x) }))
    {
        return "`"$Value`""
    }
...
Function GetFileDate {
    Get-Date -f yyMMdd-hhmmssfffffff
}

Function SetPropertiesFromHash {
    ...
    function FlattenHash {
        ...
        {
            if($PSCmdlet.ShouldProcess($Context, "Test for property $key")){
                $propArgs = @{ Path = $Context }
                if($prefix) { $propArgs.Name = $prefix }
                $found = Get-ItemProperty @propArgs | Get-Member -Name $key
                $nextKey = $found | ForEach-Object Name -ErrorAction SilentlyContinue
                if(-not $nextKey) {
                    $nextKey = $found | ForEach-Object ElementTagNam -ErrorAction SilentlyContinue
                }
                if(-not $nextKey) {
                    throw "Could not resolve property '$Prefix.$key' for item $On"
                }
            } else {
                Write-Warning "Cannot confirm property '$key' exists for non-existing path '$On'"
                $nextKey = $key
            }

            $nextPrefix = $nextKey
            if($Prefix) { $nextPrefix = "$Prefix.$nextPrefix" }
            if($Values.$key -is [System.Collections.IDictionary]) {
                $chain = FlattenHash -Values $Values.$key -Context $Context -Prefix $nextPrefix
                foreach($entry in $chain.keys) {
                    $results[$entry] = $chain.$entry
                }
            } else {
                $results[$nextPrefix] = $Values.$key
            }
        }
    ...
    try {
        $flattened = FlattenHash -Values $Properties -Context $On

        foreach($key in $flattened.Keys){
            $value = $flattened.$key
            if($PSCmdlet.ShouldProcess($On, "Set property $key => $value")) {
                # If the key contains 'password' do not write value
                if($key -notlike '*password*') {
                    Write-TaskInfo -Message "$key => $value" -Tag 'Setting'
                }
                Set-ItemProperty -Path $On -Name $key -Value $value
            }
        }
    } catch {
        Write-Error $_
...
function IsAdmin {
    param(
        [Security.Principal.WindowsPrincipal]$User = (GetUser)
    )

    return HasBuiltInRole -User $User -Role 'Administrator'
}

function HasBuiltInRole {
    param(
        [Parameter(Mandatory=$true)]
...
function GetUser {
    ...
    if($UserId){
        try {
            return [Security.Principal.WindowsIdentity]::new($UserId)
        } catch {
...
function ResolveLogPath {
    ...
    $filename = "$basename.{0:yyMMdd}" -f (Get-Date)
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUVVb2jaSceoj2MEcI6rUpk9sT
# xSegghL8MIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# MAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFI+CsgObgCcEeMV7bw3so6/k
# JCAqMA0GCSqGSIb3DQEBAQUABIIBAD/DrREgSvdWtqqTdiSFjxMZHjCVYPbIe1Yy
# PiN+9YPOEOzgsw5xRrVwnR+B9OYG1tCl6unoHHW5g3mJwy7bKJzSs802AawrBfFw
# +zxziHq2E7XxTUwJ6xFxqpZ0/6x3y9zAaEmioDpPJnJRi5zXvwdlfBPtiQUNsVlZ
# ft2jYBINNtCWPvI5/3Kaqx8r/eOnjPRj94F4MMOTLf52chLXNTrTFwAUiLY+nB0F
# Id1nAssKPbbug0sQhJeQaGtOZOB6AnzB4GXglYHLkBdglKrD/0ea1YUAy9A6uxu1
# veud2axUgY4jtMJspIjWxtXejGD1T6vHei+UYI4SPwPPr0ZeCfihggILMIICBwYJ
...
# MTgwMjEyMDgzMzU4WjAjBgkqhkiG9w0BCQQxFgQUEkwkMDIYHSTqWF3QNeM0uEIb
# DkYwDQYJKoZIhvcNAQEBBQAEggEAGnhapPF6ASX82/Qxvm+nOGwvP5E33Z97FDB2
# jC3LiGtsgAAhKvY3ytDzOQ6jYq0xOTXEqHX8FpMxt+2bMhu4k+9wXjHH87xTyvUN
# qjrit15WakA3eKQLxZX7CEtKZAiB6J5JI/fx1Wrct3BQmVze99bov6LFKfKFve1k
# P4LiTGzxdCmsmJSnArVsLnKzWRwNzJDvHBgmpnfHKuqD6eCzdeRZoZwLX3lCEjYm
# 7CllTMntMzWJ/hK5sH3Nz5eG6E3FWPNUXll0y1d21SGJiStdc4NMtd/51AkLT3x1
# COdpE+suLNfSLtlLuQjBqLS/Q1JObzqgw2+zTTwkW7pJUyalEQ==

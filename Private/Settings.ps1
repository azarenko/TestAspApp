...
function MergeSettings {
    ...
    if(!$Config) { $Config = [PSCustomObject]@{}}
    if(!$Command) { $Command = @{} }
    ...
    $settingsMap = @{
        ErrorAction  = @{
            Validate = {
                param($value)
                return [System.Management.Automation.ActionPreference]$value
            }
            Default = [System.Management.Automation.ActionPreference]::Stop
        }
        WarningAction  = @{
            Validate = {
                param($value)
            ...
            Default = [System.Management.Automation.ActionPreference]::Continue
        }
        InformationAction  = @{
            Validate = {
                param($value)
    ...
    try {
        # if key exists in command, wins - else Config
        foreach($key in $settingsMap.Keys){
            if($Command.ContainsKey($key)) {
                try {
                    $results.$key = & $settingsMap.$key.Validate $Command.$key
                } catch {
                    throw "Setting '$key' could not be processed: $($_.Exception.Message)"
                }
            } elseif($Config.HasMember($key)){
                try {
                    $results.$key = & $settingsMap.$key.Validate $Config.GetMember($key)
                } catch {
                    throw "Setting '$key' from configuration could not be processed: $($_.Exception.Message)"
                }
            } else {
                $results.$key = $settingsMap.$key.Default
            }
        }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUiHqbkuj0njDWZdaGpYZnLQ7X
# 0qqgghL8MIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# MAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFKUhfFGpvMDzbyl3bn7oCEU1
# YukDMA0GCSqGSIb3DQEBAQUABIIBAKKP7a5R24ZVEWricGCvxhxGe/JlZCqYNv4L
# ClrMddXlEb8cU1QtFqB/qTr1yUNhWOiPqy1WyGNvtTrRRzh6VcUK2cwUKLz6koKV
# k104mGa+lggWbKdyJ1cuCa0ggS63T3R1CzBR8ZbSbZPNTm8k29nlPuA6m0d1WMO7
# qrRywRETB7DbQgKHvaJMua5w8nzxxFpvND3Ytgzn4TyCtASlnI4nTGvKF5JJduWD
# 5b3xx39p6xVFxuzCvLfB2PcSfV8nhJ1Gd+cLhGts+eJHbJkoap0j53wh+iBNoIfN
# TSh2KB7FvUTnqpWKXaid19nrUr4BoYoqgAhQneiKC8/ecVqkFwyhggILMIICBwYJ
...
# MTgwMjEyMDgzMzU3WjAjBgkqhkiG9w0BCQQxFgQU5Q5ZvXDyd/RtdfGHfx85PIpb
# XKswDQYJKoZIhvcNAQEBBQAEggEAXu3EBPFiOtJkmtHitBZ/vnKVdeXh+1ZBsXcc
# gC39TTYYnCJfjmhrCBaLACCdFZmf6D1AK5OuA8XJaagYaTOXAZX3jbAiWg1bsAmy
# hjjYf1tQiO+J7jyzZmDM4euPxwJhod+BZr1US4sArDmZHmZpmq4uGhxv9i++XnXI
# hPr41lphOEN0LInHUPFj3IRlrld6ZiJD3x/AAWrgsoLWzoPIe28tbRd0TIGFjyN6
# ipNemFbue8mEspVh9bkO4BQSCfMZ9IZyYU1N7mGxym+Fsus5lWIOqPja9R/GkHug
# SpsNwTCSCWEtxHBL+Ktltx0R2cMvoAnX6Y2Xhgm7K41O7cYVyA==

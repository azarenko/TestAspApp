...
Function GetConfigFunctionResult {
    param(
    ...
    $program = $null
    try {
        $tokens = Tokenize -Value $FunctionText
        $syntaxTree = Parse -Tokens $tokens
        $program = Compile -Ast $syntaxTree

        # Add each parameter explicitly to the program
        $parametersVar = [PSVariable]::new("parameters", $Parameters)
        $program.Parameters.Add($parametersVar)
        $variablesVar = [PSVariable]::new("variables", $variables)
        $program.Parameters.Add($variablesVar)
        $block = [scriptblock]::Create($program.Script)

        $block.InvokeWithContext($null, $program.Parameters)
    }
    catch {
        Write-Error -Message "Unable to parse $FunctionText - $($_.Exception.Message)"
    }
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUs2fvD/nT/z1OyEVah/7+HlQd
# U7ygghL8MIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# MAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFPcgHLnPZNl1QEqftqvbHnDt
# kulyMA0GCSqGSIb3DQEBAQUABIIBAJKb7IkJtsaH/M4QLlLpfENXFDOPjNR8IneO
# pDxiS7T5M/7XMMYe4ui82R7PNsnMSnP7npMl4cMslztAeU70KBEb8+GEboAlHFPx
# Q0cNamZ4QJmOl9fA4M2uDGcuexqtQLVNGSVSNudBdUahc3ISA4xWKxyQghJERVJy
# pTNfM4+1ye/DOdfCTpO44fvnwTfXSUXfXbm2Z5UZh5TKJpen9n/Nun6NQjgBY1qR
# aVYZrZGa1uZGYH0l9uoGTcoq0cke40T2ksa48z77Gl5P1Q5E43665bs/NF/CsqGI
# JhoF1xjyJ0iFG7n7inR0xtP/3BafnTj3egKGXKRpSeg71W/qafyhggILMIICBwYJ
...
# MTgwMjEyMDgzNDAyWjAjBgkqhkiG9w0BCQQxFgQUJvw51rr3esCbMxYYJ6PvHjjZ
# 1BcwDQYJKoZIhvcNAQEBBQAEggEAkt+HBYcu0VuD1Rg8JfFSeLYzUt2E1bTz+GWw
# c68cTHODZ3MwlLW9AwnSsiZ0IONN3CgtBPm0AxCcS2FsWUYE5drBOZU+nwMxpKl5
# lMqMqPrSQCjTnv+SkH/RVIrySdVr9ScjMoP4dAn9dxoj5N3d80lknx57nTJ38DKk
# F3WuC0JCx2h97S/wa/Xga3+I+6/WIagF1mVLNyzCHWLw4pBW3U8APUij0y3I9M8K
# NDTjXtKZDdVqx9nr8JPmTp+bHE4F7xoxsmaBBFPceMrNQ+iJ/g7A3muKihW1ncrY
# dtR1Io3JQl9vCUz4skYdPAbYWFx1cFLImTSu230SF8NZIRCQYA==

    ...
    if($_ = GetSection -InputObject $InputObject -Section 'Parameters'){
        ...
        foreach($member in $_.GetMembers()){
            if(-not($processed.Keys -contains $member.Name)){
                ...
                if($hasDefault){
                    $processed[$member.Name] = $member.Value.DefaultValue
                } elseif($hasReference){
                    $references[$member.Name] = $member.Value.Reference
                }

                # else the parameter is not processed
            }
        }

        $processed = DereferencePointers -References $references -Processed $processed
    }
...
function DereferencePointers {
    ...
    function ProcessKey {
        ...
        if($Processed.Keys -contains $pointer) {
            # Update every entry in chain
            $Chain.ForEach({
                $processed[$_] = $Processed[$pointer]
                $References.Remove($_)
            })
...
function MapTasks {
    ...
    try {
        ...
        if($_ = GetSection -Section 'Tasks' -InputObject $InputObject) {
...
function EvaluateObject {
    ...
    if($InputObject -is [string]){
        if(ValidateConfigFunctionFormat -FunctionText $InputObject){
            return GetConfigFunctionResult -FunctionText $InputObject -Parameters $Parameters -Variables $Variables
        }
        return $InputObject
    }
    ...
    if($InputObject -is [psobject]) {
        ...
        foreach($member in $members) {
            $result[$member.Name] = EvaluateObject $member.Value -Parameters $Parameters -Variables $Variables
        }
    ...
    if($InputObject -is [System.Collections.IEnumerable]){
        ...
        foreach ($object in $InputObject) {
            $evaluated = EvaluateObject $object -Parameters $Parameters -Variables $Variables
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUT5qKEnysbMSagwR8Lnk5KgUg
# C52gghL8MIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# MAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFEPhX2eL9FNGx9PHpP8IHld0
# I3HgMA0GCSqGSIb3DQEBAQUABIIBAEDd2FtwNbQYoCVd1XNKOc7GTYx6dipZI/QB
# TzOXUBI8VyvB6r6+2GKpFg8xwPAjpFIzKko6bgWLlhsLQU4+5eYPdkVF3NrnelxI
# HTnQF6YLbpcXYjvoOxPElWdDUmJCCHKb8oyvdTnBBRokTy80PGtG44ow4bwlqLYA
# LgBk5/ZFSH9j5o40oJMx8xvCwQWxm4tetecpdW4xTKMVzHqn9AqDlY2yzXPjLjZf
# Fi98KQs70ooxpyvQSjdsaRPfc0eDbWx8+RawX7WYJ7HqO8tRj5qQ2yoLftqbD4D4
# xIppZHEq/hjo7ZE5LeMicsNUrkOuSeIq9GlV63e2LaoEFpQPQt6hggILMIICBwYJ
...
# MTgwMjEyMDgzNDAyWjAjBgkqhkiG9w0BCQQxFgQUw1xqnMsbIUL/Y6XfYRJ+t/Lh
# RVowDQYJKoZIhvcNAQEBBQAEggEAO0paAocrvJ5yJlCefXNSmQymyeZEEfQH6XyN
# rk710SuvNpcVUO8RLIo+sFUv1x5NR8UdEXkQWfLKaTABFVHAe5NhhTPl4IF+pMsX
# 0lEK1vais/gaMmy+c3m9rfSeRRwoJNb6rnPuLw+h/Mv9k5UlTYotlyXAhpnWfZ/F
# Dn9hz9LAoS2C5ABHIF5rle3Curwxxvgalun4VyGbb3tXl6fsc9+YZH3IdcYP0hLA
# +oJVWeo1V0Fxmw3BBGYqZiO363IhZhFL3kyhd8UL9jO0wc1pJRq23LZW3sQnYpFY
# bLq6Fr8PH7Ruv2XOfYD4icXOPNlRKSqWKh708caOGBZaZbxD/Q==
